...
export const folderService = {
    async getFolders(userId: string) {
        const result = await pool.query(`SELECT folders.id, folders.name, folders.color, folders.created_at, folders.updated_at, users.full_name FROM folders INNER JOIN users ON folders.owner_id = users.id WHERE folders.owner_id = $1`, [userId]);
        return result.rows;
    },
    ...
    async createFolder(userId: string, name: string, color: string) {
        ...
        `INSERT INTO folders (id, name, color, owner_id, created_at, updated_at)
